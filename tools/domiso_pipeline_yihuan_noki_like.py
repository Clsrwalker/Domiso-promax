...
LEAD_FOCUS_MAX = 82
YIHUAN_LAYOUT = (
    "Noki-like chromatic 36-key window C2-B4; "
    "DoMiSo text uses octave prefixes and #/b accidentals"
)
...
PROFILES: Dict[str, Profile] = {
    "yihuan_noki_like": Profile(
        "yihuan_noki_like",
        "Yihuan Noki-like 36-key: maximize coverage in C2-B4, octave-fold overflow, keep DoMiSo arrangement",
        -36,
        36,
        0.05,
        False,
    ...
    "yihuan_noki_like_dense": Profile(
        "yihuan_noki_like_dense",
        "Yihuan dense Noki-like 36-key: maximize C2-B4 coverage while retaining more harmony",
        -36,
        36,
        0.04,
        False,
...
def recommend_profile(metrics: dict) -> Tuple[str, List[str]]:
    ...
    if metrics["max_poly"] >= 7 or metrics["bar_density_p90"] >= 18:
        reasons.append("dense piano texture -> yihuan_noki_like_dense")
        return "yihuan_noki_like_dense", reasons
    reasons.append("default Yihuan Noki-like 36-key profile")
    return "yihuan_noki_like", reasons
...
def choose_base_shift_profile(
    ...
    # Noki-like mode uses the same idea as "maximum coverage centered octave
    # direct crop": choose an octave shift that puts the largest weighted body
    # of the source inside one 36-semitone window, then octave-fold overflow.
    candidates = [s for s in (-36, -24, -12, 0, 12, 24, 36) if p.shift_min <= s <= p.shift_max]
    ...
    for s in sorted(set(candidates), key=lambda x: (abs(x), x)):
        ...
        for idx, note in enumerate(notes):
            shifted = note["note"] + s
            if PLAYABLE_MIN <= shifted <= PLAYABLE_MAX:
                continue
            weight = 1.0
            if idx in top_ids:
                weight += 1.0
            if note["track"] == melody_track:
                weight += 0.55
            coverage_penalty += weight
        c = evaluate_shift_cost_yihuan(notes, s, top_ids, melody_track, tpb)
        c += coverage_penalty * 0.9 + p.shift_bias * abs(s)
        if c < best_c:
            best_s, best_c = s, c
    return best_s
...
def assign_support_voices(
    ...
    for step in sorted(by_start):
        ...
        filtered = []
        for n in arr:
            # Noki-like mode keeps much more of the source than melodylock. Only
...
def next_report_path(report_dir: str, midi_base: str) -> str:
    ...
    pat = re.compile(
        re.escape(safe_base) + r"_analysis_yihuan_noki_like_v(\d+)\.md$",
        re.I,
    )
    mx = 0
    ...
    return os.path.join(song_dir, f"{safe_base}_analysis_yihuan_noki_like_v{mx + 1}.md")
...
def write_report(path: str, input_midi: str, metrics: dict, profile: str, reasons: List[str], extra: dict) -> None:
    ...
    lines = [
        f"# Analysis (Yihuan Noki-Like 36-Key Script): {os.path.basename(input_midi)}",
        "",
        "## Metrics",
        ...
        *[f"- reason: {r}" for r in reasons],
        "",
        "## Yihuan Noki-Like 36-Key Intent",
        "- preserve literal rhythm/body more aggressively than melodylock",
        "- preserve semitones as DoMiSo #/b tokens instead of snapping everything to white keys",
        "- keep Voice A as the lead anchor but retain more source harmony in B/C",
        "- choose an octave shift for maximum coverage in C2-B4, then octave-fold overflow",
...
def convert_midi(input_midi: str, out_dir: str, profile_name: str) -> dict:
    ...
    out_path = base.next_script_output_path(out_dir, midi_base, "script_yihuan_noki_like")
    ...
    text = "\n".join(
        [
            ...
            f"Info: Noki-like strategy: maximum C2-B4 coverage, octave-fold overflow; windows: {summary}",
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Analyze + profile-select + Yihuan Noki-like 36-key DoMiSo conversion workflow."
    )
    ...
    legacy = argparse.ArgumentParser(description="legacy Yihuan Noki-like 36-key convert")
