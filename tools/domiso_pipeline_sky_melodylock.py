...
PROFILES: Dict[str, Profile] = {
    "sky_melodylock": Profile(
        "sky_melodylock",
        "Sky 15-key melodylock: monophonic lead, center the hook, thin support to fit C4-C6",
        -12,
        12,
        0.34,
        True,
    ...
    "sky_melodylock_dense": Profile(
        "sky_melodylock_dense",
        "Sky 15-key melodylock dense: preserve body carefully while keeping the lead recognizable",
        -12,
        12,
        0.26,
        True,
...
def recommend_profile(metrics: dict) -> Tuple[str, List[str]]:
    ...
    if metrics["max_poly"] >= 6 or metrics["bar_density_p90"] >= 16:
        reasons.append("15-key range + dense source -> sky_melodylock_dense")
        return "sky_melodylock_dense", reasons
    reasons.append("default Sky melodylock profile")
    return "sky_melodylock", reasons
...
def assign_support_voices(
    ...
    for step in sorted(by_start):
        ...
        filtered = []
        for n in arr:
            if n["dur_steps"] <= 1 and not strong and n["vel"] < 80 and n["track"] != melody_track:
                pruned += 1
                continue
            if dense and not strong and n["dur_steps"] <= 2 and not n["is_top"] and n["track"] != melody_track:
                pruned += 1
                continue
        ...
        if not bass_candidates:
            bass_candidates = [0]
        keep_bass = strong or len(filtered) <= 2
        keep_bass = keep_bass or any(filtered[i]["dur_steps"] >= 6 for i in bass_candidates)
        keep_bass = keep_bass or (eighth and any(filtered[i]["dur_steps"] >= 4 for i in bass_candidates))
        ...
        if strong:
            keep = 1
        elif eighth and not dense and any(n["dur_steps"] >= 3 for _, n in remaining):
            keep = 1
...
def next_report_path(report_dir: str, midi_base: str) -> str:
    ...
    pat = re.compile(
        re.escape(safe_base) + r"_analysis_sky_melodylock_v(\d+)\.md$",
        re.I,
    )
    mx = 0
    ...
    return os.path.join(song_dir, f"{safe_base}_analysis_sky_melodylock_v{mx + 1}.md")
...
def write_report(path: str, input_midi: str, metrics: dict, profile: str, reasons: List[str], extra: dict) -> None:
    ...
    lines = [
        f"# Analysis (Sky MelodyLock Script): {os.path.basename(input_midi)}",
        "",
        "## Metrics",
...
def convert_midi(input_midi: str, out_dir: str, profile_name: str) -> dict:
    ...
    out_path = base.next_script_output_path(out_dir, midi_base, "script_sky_melodylock")
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Analyze + profile-select + Sky 15-key melodylocked literal DoMiSo conversion workflow."
    )
