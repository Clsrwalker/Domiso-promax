...
def convert_midi(input_midi: str, out_dir: str, profile_name: str):
    ...
    out_path = next_script_output_path(out_dir, base, "script_longnote")
...
def next_report_path(report_dir: str, midi_base: str) -> str:
    ...
    os.makedirs(song_dir, exist_ok=True)
    pat = re.compile(re.escape(safe_base) + r"_analysis_longnote_v(\d+)\.md$", re.I)
    mx = 0
    for fn in os.listdir(song_dir):
        m = pat.match(fn)
        if m:
            mx = max(mx, int(m.group(1)))
    return os.path.join(song_dir, f"{safe_base}_analysis_longnote_v{mx + 1}.md")


def write_report(path: str, input_midi: str, metrics: dict, profile: str, reasons: List[str]):
    lines = [
        f"# Analysis (Longnote Script): {os.path.basename(input_midi)}",
        "",
        "## Metrics",
