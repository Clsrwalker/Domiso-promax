...
PROFILES: Dict[str, Profile] = {
    "yihuan_ballad36": Profile(
        "yihuan_ballad36",
        "Yihuan 36-key ballad36: vocal-like middle lead, exact semitones, soft accompaniment",
        -12,
        12,
        0.08,
        False,
    ...
    "yihuan_ballad36_dense": Profile(
        "yihuan_ballad36_dense",
        "Yihuan 36-key dense ballad36: vocal-like middle lead, richer but controlled accompaniment",
        -12,
        12,
        0.06,
        False,
...
def recommend_profile(metrics: dict) -> Tuple[str, List[str]]:
    ...
    if metrics["max_poly"] >= 7 or metrics["bar_density_p90"] >= 18:
        reasons.append("dense piano texture -> yihuan_ballad36_dense")
        return "yihuan_ballad36_dense", reasons
    reasons.append("default Yihuan 36-key ballad36 profile")
    return "yihuan_ballad36", reasons
...
def choose_base_shift_profile(
    ...
    # Ballad36 is for Yihuan's chromatic instrument. Unlike 21-key scripts,
...
def score_lead_note(
    ...
    if BALLAD_MID_MIN <= note["pitch"] <= BALLAD_MID_MAX:
        score += 1.0
    elif BALLAD_MID_MAX < note["pitch"] <= BALLAD_SOFT_HIGH_MAX:
        score += 0.25
    elif note["pitch"] > BALLAD_SOFT_HIGH_MAX:
        score -= min(3.0, (note["pitch"] - BALLAD_SOFT_HIGH_MAX) * 0.25)
    else:
        score -= min(2.2, (BALLAD_MID_MIN - note["pitch"]) * 0.18)
...
def midlead_register_penalty(pitch: int) -> float:
    if MIDLEAD_MIN <= pitch <= MIDLEAD_MAX:
        return 0.0
    if MIDLEAD_MAX < pitch <= MIDLEAD_SOFT_HIGH_MAX:
        return 1.0 + (pitch - MIDLEAD_MAX) * 0.35
    if pitch > MIDLEAD_SOFT_HIGH_MAX:
        return 3.0 + (pitch - MIDLEAD_SOFT_HIGH_MAX) * 0.8
    if 55 <= pitch < MIDLEAD_MIN:
        return 0.7 + (MIDLEAD_MIN - pitch) * 0.18
    return 2.0 + (MIDLEAD_MIN - pitch) * 0.45


def center_lead_intervals_midrow(
    intervals: List[Tuple[int, int, int]]
) -> Tuple[List[Tuple[int, int, int]], dict]:
    centered: List[Tuple[int, int, int]] = []
    stats = {
        "lead_midlead_moved": 0,
        "lead_midrow_notes": 0,
    ...
    for start, end, pitch in sorted(intervals, key=lambda x: (x[0], x[1], x[2])):
        ...
        for cand in candidates:
            offset = cand - pitch
            score = midlead_register_penalty(cand)
            score += 0.025 * abs(offset)

            if prev_pitch is not None:
                score += 0.045 * abs(cand - prev_pitch)
                if prev_offset is not None and offset != prev_offset:
                    score += 1.15

            # If the source is already in a comfortable register, avoid moving it
            # unless another octave is clearly better.
            if offset == 0 and MIDLEAD_MIN <= pitch <= MIDLEAD_SOFT_HIGH_MAX:
                score -= 0.45
        ...
        if chosen != pitch:
            stats["lead_midlead_moved"] += 1
        if MIDLEAD_MIN <= chosen <= MIDLEAD_MAX:
...
def assign_support_voices(
    ...
    for step in sorted(by_start):
        ...
        filtered = []
        for n in arr:
            # Ballad36 deliberately simplifies accompaniment into bass + harmonic
            # pad so the vocal line reads clearly on Yihuan.
...
def next_report_path(report_dir: str, midi_base: str) -> str:
    ...
    pat = re.compile(
        re.escape(safe_base) + r"_analysis_yihuan_ballad36_v(\d+)\.md$",
        re.I,
    )
    mx = 0
    ...
    return os.path.join(song_dir, f"{safe_base}_analysis_yihuan_ballad36_v{mx + 1}.md")
...
def write_report(path: str, input_midi: str, metrics: dict, profile: str, reasons: List[str], extra: dict) -> None:
    ...
    lines = [
        f"# Analysis (Yihuan 36-Key Ballad36 Script): {os.path.basename(input_midi)}",
        "",
        "## Metrics",
        ...
        *[f"- reason: {r}" for r in reasons],
        ...
        f"- lead_midlead_moved: {extra['midlead_stats']['lead_midlead_moved']}",
        f"- lead_midrow_notes: {extra['midlead_stats']['lead_midrow_notes']}",
        f"- lead_highrow_notes: {extra['midlead_stats']['lead_highrow_notes']}",
...
def convert_midi(input_midi: str, out_dir: str, profile_name: str) -> dict:
    ...
    lead_intervals, midlead_stats = center_lead_intervals_midrow(lead_intervals)
    lead_intervals, lead_ornaments_dropped = smooth_ballad_lead(lead_intervals)
    ...
    out_path = base.next_script_output_path(out_dir, midi_base, "script_yihuan_ballad36")
    ...
    text = "\n".join(
        [
            ...
            "Info: lead register strategy: Voice A prefers Yihuan middle row C4-B4; octave moves only",
            f"Info: ballad smoothing -> lead_ornaments_dropped={lead_ornaments_dropped}, support_pruned={support_pruned}",
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Analyze + profile-select + Yihuan 36-key ballad DoMiSo conversion workflow."
    )
    ...
    if args.cmd == "pipeline":
        ...
        write_report(
            ...
            reasons,
            {
                ...
                "midlead_stats": res["midlead_stats"],
    ...
    legacy = argparse.ArgumentParser(description="legacy Yihuan 36-key ballad36 convert")
