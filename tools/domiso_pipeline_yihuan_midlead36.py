...
PROFILES: Dict[str, Profile] = {
    "yihuan_midlead36": Profile(
        "yihuan_midlead36",
        "Yihuan 36-key midlead36: middle-row lead, exact semitones, high source preservation",
        -12,
        12,
        0.08,
        False,
    ...
    "yihuan_midlead36_dense": Profile(
        "yihuan_midlead36_dense",
        "Yihuan 36-key dense midlead36: middle-row lead, preserve maximum chromatic body",
        -12,
        12,
        0.06,
        False,
...
def recommend_profile(metrics: dict) -> Tuple[str, List[str]]:
    ...
    if metrics["max_poly"] >= 7 or metrics["bar_density_p90"] >= 18:
        reasons.append("dense piano texture -> yihuan_midlead36_dense")
        return "yihuan_midlead36_dense", reasons
    reasons.append("default Yihuan 36-key midlead36 profile")
    return "yihuan_midlead36", reasons
...
def choose_base_shift_profile(
    ...
    # MidLead36 is for Yihuan's chromatic instrument. Unlike 21-key scripts,
...
def assign_support_voices(
    ...
    for step in sorted(by_start):
        ...
        filtered = []
        for n in arr:
            # MidLead36 keeps much more of the source than melodylock. Only
            # remove weak, very short clutter that tends to become machine-gun taps.
            if n["dur_steps"] <= 1 and not strong and not eighth and n["vel"] < 52 and n["track"] != melody_track:
                pruned += 1
                continue
            if dense and not strong and not eighth and n["dur_steps"] <= 1 and n["vel"] < 64 and not n["is_top"] and n["track"] != melody_track:
                pruned += 1
                continue
            if lead_pitch is not None and n["track"] != bass_track and n["pitch"] == lead_pitch:
                pruned += 1
                continue
            filtered.append(n)
        if not filtered:
        ...
        if strong:
            keep = 4 if len(remaining) >= 6 else (3 if len(remaining) >= 4 else (2 if len(remaining) >= 2 else 1))
        elif eighth:
            keep = 2 if (not dense and len(remaining) >= 3) else 1
        elif len(remaining) == 1 and remaining[0][1]["dur_steps"] >= 3:
            keep = 1
        else:
            keep = 0
        if keep <= 0:
...
def next_report_path(report_dir: str, midi_base: str) -> str:
    ...
    pat = re.compile(
        re.escape(safe_base) + r"_analysis_yihuan_midlead36_v(\d+)\.md$",
        re.I,
    )
    mx = 0
    ...
    return os.path.join(song_dir, f"{safe_base}_analysis_yihuan_midlead36_v{mx + 1}.md")
...
def write_report(path: str, input_midi: str, metrics: dict, profile: str, reasons: List[str], extra: dict) -> None:
    ...
    lines = [
        f"# Analysis (Yihuan 36-Key MidLead36 Script): {os.path.basename(input_midi)}",
        "",
        "## Metrics",
...
def convert_midi(input_midi: str, out_dir: str, profile_name: str) -> dict:
    ...
    out_path = base.next_script_output_path(out_dir, midi_base, "script_yihuan_midlead36")
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Analyze + profile-select + Yihuan 36-key middle-lead DoMiSo conversion workflow."
    )
    ...
    legacy = argparse.ArgumentParser(description="legacy Yihuan 36-key midlead36 convert")
