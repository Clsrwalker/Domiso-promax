...
PROFILES: Dict[str, Profile] = {
    "yihuan_restore36": Profile(
        "yihuan_restore36",
        "Yihuan 36-key restore36: original key, exact semitones, high source preservation",
        -12,
        12,
        0.08,
        False,
    ...
    "yihuan_restore36_dense": Profile(
        "yihuan_restore36_dense",
        "Yihuan 36-key dense restore36: original key, preserve maximum chromatic body",
        -12,
        12,
        0.06,
        False,
...
def recommend_profile(metrics: dict) -> Tuple[str, List[str]]:
    ...
    if metrics["max_poly"] >= 7 or metrics["bar_density_p90"] >= 18:
        reasons.append("dense piano texture -> yihuan_restore36_dense")
        return "yihuan_restore36_dense", reasons
    reasons.append("default Yihuan 36-key restore36 profile")
    return "yihuan_restore36", reasons
...
def choose_base_shift_profile(
    ...
    # Restore36 is for Yihuan's chromatic instrument. Unlike 21-key scripts,
...
def assign_support_voices(
    ...
    for step in sorted(by_start):
        ...
        filtered = []
        for n in arr:
            # Restore36 keeps much more of the source than melodylock. Only
...
def next_report_path(report_dir: str, midi_base: str) -> str:
    ...
    pat = re.compile(
        re.escape(safe_base) + r"_analysis_yihuan_restore36_v(\d+)\.md$",
        re.I,
    )
    mx = 0
    ...
    return os.path.join(song_dir, f"{safe_base}_analysis_yihuan_restore36_v{mx + 1}.md")
...
def write_report(path: str, input_midi: str, metrics: dict, profile: str, reasons: List[str], extra: dict) -> None:
    ...
    lines = [
        f"# Analysis (Yihuan 36-Key Restore36 Script): {os.path.basename(input_midi)}",
        "",
        "## Metrics",
...
def convert_midi(input_midi: str, out_dir: str, profile_name: str) -> dict:
    ...
    out_path = base.next_script_output_path(out_dir, midi_base, "script_yihuan_restore36")
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Analyze + profile-select + Yihuan 36-key restore-first DoMiSo conversion workflow."
    )
    ...
    legacy = argparse.ArgumentParser(description="legacy Yihuan 36-key restore36 convert")
