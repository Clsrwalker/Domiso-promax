...
PROFILES: Dict[str, Profile] = {
    "yihuan_melodylock": Profile(
        "yihuan_melodylock",
        "Yihuan 36-key melodylock: chromatic lead, exact semitones, restrained support",
        -12,
        12,
        0.34,
        True,
    ...
    "yihuan_melodylock_dense": Profile(
        "yihuan_melodylock_dense",
        "Yihuan 36-key dense melodylock: preserve more chromatic body while keeping lead singable",
        -12,
        12,
        0.24,
        True,
...
def recommend_profile(metrics: dict) -> Tuple[str, List[str]]:
    ...
    if metrics["max_poly"] >= 7 or metrics["bar_density_p90"] >= 18:
        reasons.append("dense piano texture -> yihuan_melodylock_dense")
        return "yihuan_melodylock_dense", reasons
    reasons.append("default Yihuan 36-key melodylock profile")
    return "yihuan_melodylock", reasons
...
def next_report_path(report_dir: str, midi_base: str) -> str:
    ...
    pat = re.compile(
        re.escape(safe_base) + r"_analysis_yihuan_melodylock_v(\d+)\.md$",
        re.I,
    )
    mx = 0
    ...
    return os.path.join(song_dir, f"{safe_base}_analysis_yihuan_melodylock_v{mx + 1}.md")
...
def write_report(path: str, input_midi: str, metrics: dict, profile: str, reasons: List[str], extra: dict) -> None:
    ...
    lines = [
        f"# Analysis (Yihuan 36-Key MelodyLock Script): {os.path.basename(input_midi)}",
        "",
        "## Metrics",
...
def convert_midi(input_midi: str, out_dir: str, profile_name: str) -> dict:
    ...
    out_path = base.next_script_output_path(out_dir, midi_base, "script_yihuan_melodylock")
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Analyze + profile-select + Yihuan 36-key melodylocked DoMiSo conversion workflow."
    )
    ...
    legacy = argparse.ArgumentParser(description="legacy Yihuan 36-key melodylock convert")
