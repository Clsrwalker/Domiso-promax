...
Decrypt_file(f)
{
	...
	; RawRead fills byte buffers. Convert UTF-16LE bytes back to normal AHK strings
	; so encrypted DMS files generated by the web converter can be parsed reliably.
	plaintext_text := StrGet(&plaintext, plaintext_len//2, "UTF-16")
	ciphertext_text := StrGet(&ciphertext, ciphertext_len//2, "UTF-16")
	Return [plaintext_text, ciphertext_text]
}
